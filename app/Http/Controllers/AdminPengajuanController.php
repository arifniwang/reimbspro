...
class AdminPengajuanController extends \crocodicstudio\crudbooster\controllers\CBController
{
	...
	public function cbInit()
	{
		...
		*/
		...
		$this->button_selected[] = ['label' => 'Approve Terpilih', 'icon' => 'fa fa-check', 'name' => 'approve'];
		$this->button_selected[] = ['label' => 'Reject Terpilih', 'icon' => 'fa fa-ban', 'name' => 'reject'];
	...
	*/
	public function actionButtonSelected($id_selected, $button_name)
	{
		if ($button_name == 'approve') {
			...
			if (count($pengajuan) != count($id_selected)) {
				CRUDBooster::redirectBack('Data pengajuan yang anda pilih tidak dapat di approve, pastikan memilih data yang sesuai');
			} else {
				/**
				 * UPDATE STATUS
				 */
				...
				$save['month'] = number_format(date('m'), 0, '', '');
				$save['status'] = 'Disetujui';
				DB::table('pengajuan')->whereIn('id', $id_selected)->update($save);
				...
				foreach ($pengajuan as $row){
					 ...
					 */
					$save_usr_notif['created_at'] = date('Y-m-d H:i:s');
					$save_usr_notif['id_pengajuan'] = $row->id;
					$save_usr_notif['title'] = 'REIMBURSEMENT BERHASIL';
					$save_usr_notif['content'] = 'Pengajuan “' . $row->name . '” berhasil dikirim dan diterima oleh administrator';
					$save_usr_notif['date'] = date('Y-m-d');
					$save_usr_notif['type'] = 'Disetujui';
					$act_notif = DB::table('users_notification')->insert($save_usr_notif);
					...
					$data_notif['content'] = $save_usr_notif['content'];
					$data_notif['id'] = $act_notif;
					$data_notif['id_pengajuan'] = $row->id;
					CRUDBooster::sendFCM($regid, $data_notif);
				}
			...
			if (count($pengajuan) != count($id_selected)) {
				CRUDBooster::redirectBack('Data pengajuan yang anda pilih tidak dapat di direject, pastikan memilih data yang sesuai');
			} else {
				$save['updated_at'] = date('Y-m-d H:i:s');
				$save['datetime_approval'] = date('Y-m-d H:i:s');
				$save['year'] = date('Y');
				...
				foreach ($pengajuan as $row){
					...
					$save_usr_notif['id_pengajuan'] = $row->id;
					$save_usr_notif['title'] = 'REIMBURSEMENT DITOLAK';
					$save_usr_notif['content'] = 'Pengajuan “' . $row->name . '” berhasil dikirim dan ditolak oleh administrator';
					$save_usr_notif['date'] = date('Y-m-d');
					$save_usr_notif['type'] = 'Ditolak';
					$act_notif = DB::table('users_notification')->insert($save_usr_notif);
